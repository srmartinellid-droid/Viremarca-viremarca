...
export default function AdminPage() {
  ...
  const [groqModels, setGroqModels] = useState<Array<{ id: string; active: boolean; label: string }>>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const supportedGroqModels = [
        { id: "auto", active: true, label: "Automático · custo inteligente" },
        { id: "openai/gpt-oss-20b", active: true, label: "GPT-OSS 20B · econômico" },
        { id: "openai/gpt-oss-120b", active: true, label: "GPT-OSS 120B · maior capacidade" },
      ]
      setGroqModels(supportedGroqModels)
      ...
      if (response.ok) {
        ...
        if (data.groq_configured) {
          ...
          if (modelsResponse.ok && Array.isArray(modelsData?.models) && modelsData.models.length > 0) setGroqModels(modelsData.models)
          else setGroqModelsError(modelsData?.error || "Usando a lista de modelos suportados enquanto a Groq não responde.")
        } else {
          setGroqModelsError("Selecione o modelo agora. A chave Groq ainda precisa ser configurada para o chat responder.")
        }
      }
