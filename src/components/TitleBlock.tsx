...
export function TitleBlock({ style, eyebrow, className = "", size = "default" }: { style: TitleStyle; eyebrow?: string; className?: string; size?: "default" | "large" }) {
  ...
  return (
    <Reveal className={`${align} ${className}`}>
      {eyebrow && <p className="vm-eyebrow mb-4">{eyebrow}</p>}
      <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.65 }} className={`vm-display text-pretty leading-[1.04] tracking-[-0.045em] ${size === "large" ? "text-4xl md:text-5xl lg:text-[3.6rem]" : "text-3xl md:text-4xl lg:text-[2.75rem]"}`} style={{ color: style.textColor || "inherit" }}>
        {parts.before}{parts.highlighted && <span className={highlightClass} style={{ color: style.highlightColor || style.textColor || "inherit", backgroundColor: style.highlightStyle === "marker" ? `${style.highlightColor || "#E07A5F"}20` : undefined }}>{parts.highlighted}</span>}{parts.after}
      </motion.h2>
    </Reveal>
  )
}
