      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 items-start">
          <Reveal>
            ...
            <h2 className="vm-display text-3xl md:text-4xl lg:text-[2.75rem] text-vm-ink leading-[1.12]">{content.aboutTitle}</h2>
          </Reveal>
          <Reveal delay={0.12} className="space-y-6">
            {paragraphs.map((p, i) => <p key={i} className="text-vm-muted leading-relaxed text-base md:text-lg">{p}</p>)}
          </Reveal>
        </div>
      </div>
    </section>
