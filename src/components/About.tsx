    ...
    <section id="quem-somos" className="relative overflow-hidden bg-vm-sand py-24 md:py-32">
      ...
      <div className="mx-auto max-w-[1400px] px-5 sm:px-8 lg:px-12">
        <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
          <Reveal><div className="sticky top-28"><p className="vm-eyebrow mb-5 flex items-center gap-3"><span className="text-vm-ink/25">02</span> Quem somos</p><TitleBlock style={title} className="!mx-0 !text-left" /><div className="mt-10 flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-vm-muted"><span className="h-2 w-2 rounded-full bg-vm-coral" />VireMarca®</div></div></Reveal>
          <Reveal delay={0.12} className="relative">
            ...
            <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-vm-ink/10 bg-vm-ink/10 md:grid-cols-3">{["Direção visual", "Engenharia", "Evolução"].map((item) => <div key={item} className="bg-vm-sand p-5 text-[10px] font-semibold uppercase tracking-[0.18em] text-vm-ink/55">{item}</div>)}</div>
          </Reveal>
        </div>
      </div>
    </section>
