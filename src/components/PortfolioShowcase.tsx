    ...
    <section id="sites" className="py-20 md:py-24 bg-white relative">
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-7 lg:px-10">
        <Reveal className="max-w-3xl mb-10">
          ...
          <p className="mt-3 text-vm-muted max-w-lg leading-relaxed">Aqui entram os projetos da VireMarca: a imagem é a captura do próprio site, e cada projeto leva para sua página de detalhes ou para a versão publicada quando disponível.</p>
        </Reveal>
              ...
              return (
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden bg-vm-sand">
                    ...
                    <div className="absolute top-4 left-4 flex flex-wrap gap-2"><span className="rounded-full bg-white/90 backdrop-blur-sm px-3 py-1 text-[11px] font-medium text-vm-ink tracking-wide">{project.category}</span><span className="rounded-full border border-white/40 bg-black/30 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white backdrop-blur-sm">Captura do site</span></div>
                  ...
                  <div className="p-5 md:p-6 bg-white">
                    <div className="flex items-start justify-between gap-3">
                      <div><h3 className="text-lg font-semibold tracking-tight text-vm-ink group-hover:text-vm-coral transition-colors">{project.title}</h3><p className="mt-2 text-sm text-vm-muted leading-relaxed line-clamp-2">{project.description}</p><p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-vm-coral">Ver projeto e contexto</p></div>
