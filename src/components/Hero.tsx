...
function HeroCard({ project, index, reduced, motionAmount, mobile = false }: { project: PortfolioProject; index: number; reduced: boolean; motionAmount: number; mobile?: boolean }) {
  const imageSrc = project.thumbnail || "/portfolio/magia-glass.jpg"
  ...
  if (mobile) return (
    <motion.a href={\`/portfolio/\${project.slug}\`} initial={reduced ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1, ease }} className="group relative w-full overflow-hidden rounded-[1.25rem] border border-white/70 bg-white shadow-[0_28px_70px_-35px_rgba(0,0,0,0.8)]">
      ...
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 text-white"><p className="text-[9px] uppercase tracking-[0.2em] text-white/60">{project.category}</p><p className="mt-1 text-lg font-semibold">{project.title}</p></div>
      </div>
    </motion.a>
  )
  ...
  return (
    <motion.a href={\`/portfolio/\${project.slug}\`} initial={reduced ? false : { opacity: 0, y: 32, scale: 0.94, rotate: index === 1 ? 2.4 : index === 2 ? -2.2 : -1.2 }} animate={reduced ? undefined : { opacity: 1, y: [(index === 1 ? 9 : -7) * motionAmount, 0], rotate: [index === 1 ? 2.4 : index === 2 ? -2.2 : -1.2, index === 1 ? 1.7 : index === 2 ? -1.5 : -0.5, index === 1 ? 2.4 : index === 2 ? -2.2 : -1.2], scale: 1 }} transition={{ opacity: { duration: 0.7, delay: 0.28 + index * 0.12, ease }, scale: { duration: 0.7, delay: 0.28 + index * 0.12, ease }, y: { duration: 7 + index, repeat: Infinity, ease: "easeInOut", delay: index * 0.4 }, rotate: { duration: 7 + index, repeat: Infinity, ease: "easeInOut", delay: index * 0.4 } }} className={\`\${positions[index] ?? positions[0]} group overflow-hidden rounded-[1.55rem] border border-white/75 bg-white shadow-[0_40px_100px_-45px_rgba(0,0,0,0.88)] transition-shadow duration-500 hover:shadow-[0_48px_110px_-42px_rgba(0,0,0,0.95)]\`}>
      ...
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute inset-x-5 bottom-5 text-white"><p className="text-[9px] uppercase tracking-[0.22em] text-white/60">{project.category}</p><p className="mt-1 text-xl font-semibold tracking-tight">{project.title}</p></div>
      </div>
    </motion.a>
  )
}
