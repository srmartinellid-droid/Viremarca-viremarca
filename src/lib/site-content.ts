...
export type PublicSiteContent = {
  heroTitle: string; heroAccent: string; heroSubtitle: string; heroAccentIntensity: number; heroImage: string; heroMobileImage: string; heroBackgroundImages: string[]; heroMobileBackgroundImages: string[]; heroOverlayIntensity: number; heroBackgroundPosition: string; heroBackgroundScale: number; heroCardsMotion: number; logo1: string; logo2: string; titleStyles: Record<string, TitleStyle>; dividerStyles: Record<string, DividerStyle>; aboutTitle: string; aboutBody: string; process: ProcessItem[]; delivers: DeliverItem[]; contact: { whatsapp: string; email: string; instagram: string }
}
...
const fallbackDividers: Record<string, DividerStyle> = {
...
export async function getPublicSiteContent(): Promise<PublicSiteContent> {
  ...
  try {
    ...
    const parseImages = (value: string | undefined, fallbackValue: string[]) => { if (!value) return fallbackValue; try { const parsed = JSON.parse(value); if (Array.isArray(parsed)) { const images = parsed.filter((item): item is string => typeof item === "string" && item.trim().length > 0); if (images.length) return images } } catch { if (value.trim()) return [value.trim()] } return fallbackValue }
    const parseJsonObject = <T,>(value: string | undefined, fallbackValue: T): T => { if (!value) return fallbackValue; try { return JSON.parse(value) as T } catch { return fallbackValue } }
